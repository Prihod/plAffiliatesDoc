...
* Автоматический редирект посетителя на основание его геолокации на нужный поддомен филиала
...
На странице "Филиалы" во вкладке "Менеджер полей филиалов" вы можете создать свои группы для полей, что в дальнейшем позволит вам в различных сниппетах как получать нужные поля передав название группы, так и использовать отдельные чанки для каждой группы полей. 
...
### Через админ-панель
Добавлять информацию по количеству оставшегося товара по филиалам можно как на отдельной странице "Пакеты" -> Филиалы" -> "Остатки товаров в филиалах", та непосредственно в карточки товара во вкладке "Остатки товара в филиалах"
...
// Количеству оставшегося товара у филиала
$remain = $tools->getAffiliateProductRemain($affiliateId, $productId);

// Общее количеству оставшегося товара по всем филиалам
...
Если вы планируете использовать автоматическое перенаправление посетителей на основании их геоданных на поддомены филиалов, то для каждого "[родительского][1]" филиала необходимо заполнить:
...
### getPlAffiliatesLinks - Вывод списка ссылок для переключения/переход на поддомен филиала.
...
[1]: Родительский филиал - филиал у которого нет родителя. У одного и того же города не может быть больше одного родительского филиала.
